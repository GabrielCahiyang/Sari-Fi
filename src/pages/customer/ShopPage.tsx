    ...
    <CustomerLayout>
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-800 text-[#0D2B45]">Sari-Fi Market</h1>
            <p className="text-sm text-[#65727A] mt-0.5">Wholesale grocery inventory for your store</p>
          </div>
          {cartCount > 0 && (
            <button
              onClick={() => navigate('customer/cart')}
              className="flex items-center gap-2 px-4 py-2.5 bg-[#1E7D3B] text-white font-600 text-sm rounded-xl hover:bg-[#22913f] transition-all"
            >
        ...
        <div className="flex gap-2 overflow-x-auto pb-3 mb-6 no-scrollbar">
          {CATEGORIES.map(cat => (
            <button
              ...
              className={`shrink-0 px-4 py-2 rounded-full text-sm font-600 transition-all border ${
                category === cat
        ...
        {/* Results count */}
        <div className="text-xs text-[#65727A] mb-4">{filtered.length} product{filtered.length !== 1 ? 's' : ''} {category !== 'All' ? `in ${category}` : ''}</div>
        ...
        {filtered.length > 0 ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {filtered.map(product => {
              ...
              return (
                <div key={product.id} className="bg-white rounded-2xl border border-[#E4E8E6] p-4 flex flex-col hover:border-[#1E7D3B]/30 hover:shadow-sm transition-all">
                  {/* Category chip */}
                  <div className="text-[10px] font-700 text-[#65727A] uppercase tracking-wider mb-2">{product.category}</div>

                  {/* Product name */}
                  <div className="font-700 text-sm text-[#10212B] leading-tight mb-1 flex-1">{product.name}</div>
                  <div className="text-xs text-[#65727A] mb-3">{product.sku}</div>

                  {/* Price */}
                  <div className="text-xl font-800 text-[#0D2B45] mb-1">{formatPHP(product.sellingPrice)}</div>
