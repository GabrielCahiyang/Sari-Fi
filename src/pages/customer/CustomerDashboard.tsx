...
export function CustomerDashboard() {
  ...
  return (
    <CustomerLayout>
      <div className="max-w-5xl mx-auto p-3.5 sm:p-6 space-y-4 sm:space-y-5">

        {/* Bento Grid */}
        <div className="grid grid-cols-12 gap-3 sm:gap-4 stagger">

          {/* Welcome Card — col 8 */}
          <div className="col-span-12 md:col-span-8 bg-gradient-to-br from-[#0D2B45] to-[#0a2237] rounded-2xl p-4 sm:p-6 relative overflow-hidden lit-top">
            ...
            <div className="relative">
              <div className="text-white/60 text-xs sm:text-sm mb-1">Welcome back,</div>
              <div className="text-white font-800 text-xl sm:text-2xl mb-0.5 truncate">{customer.fullName}</div>
              <div className="text-[#7DBE4C] text-xs sm:text-sm font-500 mb-3 sm:mb-4 truncate">{customer.storeName}</div>
              <div className="flex items-center gap-3">
                <div className="text-white/50 text-xs">Account</div>
                <div className="text-white font-700 text-xs font-mono">{customer.accountNo}</div>
              </div>
              <button
                onClick={() => navigate('customer/shop')}
                className="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-[#1E7D3B] hover:bg-[#22913f] text-white text-xs sm:text-sm font-600 rounded-xl transition-all cursor-pointer shadow-sm shadow-[#1E7D3B]/20"
              >
          ...
          <div className="col-span-12 md:col-span-4 bg-gradient-to-br from-[#22913f] to-[#1E7D3B] rounded-2xl p-4 sm:p-5 flex flex-col lit-top relative overflow-hidden">
            <div className="absolute -top-8 -right-8 w-28 h-28 bg-white/10 rounded-full" />
            <div className="text-white/70 text-xs font-600 uppercase tracking-wider">Available Credit</div>
            <div className="mt-2 text-white font-800 text-2xl sm:text-3xl truncate">{formatPHP(available)}</div>
          ...
          {/* Credit Limit */}
          <div className="card-lift col-span-6 md:col-span-4 bg-white rounded-2xl p-4 sm:p-5 border border-[#E4E8E6] shadow-soft-sm">
            <div className="text-[#65727A] text-[11px] sm:text-xs font-600 uppercase tracking-wider">Credit Limit</div>
            <div className="mt-1 sm:mt-2 text-[#0D2B45] font-800 text-xl sm:text-2xl truncate">{formatPHP(customer.creditLimit)}</div>
            <div className="flex items-center gap-1 mt-2">
              <svg className="w-3.5 h-3.5 text-[#7DBE4C] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg>
              <span className="text-[10px] sm:text-[11px] text-[#7DBE4C] font-600 truncate">+₱1k next cycle</span>
            </div>
          </div>
          ...
          <div className="card-lift col-span-6 md:col-span-4 bg-white rounded-2xl p-4 sm:p-5 border border-[#E4E8E6] shadow-soft-sm">
            ...
            <div className="mt-1 sm:mt-2 text-[#10212B] font-800 text-xl sm:text-2xl truncate">{formatPHP(totalOutstanding)}</div>
            <div className="text-[#65727A] text-[11px] mt-1 truncate">Total remaining</div>
          </div>

          {/* Next Payment */}
          <div className={`col-span-12 md:col-span-4 rounded-2xl p-4 sm:p-5 border ${nextDue ? 'bg-[#FFF8E1] border-[#FFC107]/30' : 'bg-white border-[#E4E8E6]'}`}>
            <div className="text-[#65727A] text-xs font-600 uppercase tracking-wider">Next Payment</div>
            {nextDue ? (
              <>
                <div className="mt-1 sm:mt-2 text-[#10212B] font-800 text-xl sm:text-2xl truncate">{formatPHP(nextDue.baseAmount + nextDue.penalty)}</div>
                ...
                <button
                  ...
                  className="mt-3 text-xs font-600 text-[#1E7D3B] hover:underline cursor-pointer"
                >
                  Pay now →
                </button>
              </>
              ...
              <>
                <div className="mt-2 text-[#65727A] font-600 text-base sm:text-lg">No payments due</div>
