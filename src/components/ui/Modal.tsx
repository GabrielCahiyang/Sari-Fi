...
export function Modal({ open, onClose, title, children, size = 'md' }: ModalProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 sm:p-6 overflow-y-auto" onClick={onClose}>
      <div className="fixed inset-0 bg-[#0D2B45]/50 backdrop-blur-sm" />
      <div
        className={`relative w-full ${widths[size]} max-h-[90vh] flex flex-col bg-white rounded-2xl shadow-2xl ring-1 ring-black/10 my-auto z-10`}
        onClick={e => e.stopPropagation()}
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#E4E8E6] shrink-0">
          <h2 className="text-base font-700 text-[#10212B] tracking-tight">{title}</h2>
          <button
            type="button"
            onClick={onClose}
            className="text-[#65727A] hover:text-[#10212B] transition-colors p-1.5 rounded-lg hover:bg-[#F7F8F6] cursor-pointer"
          >
